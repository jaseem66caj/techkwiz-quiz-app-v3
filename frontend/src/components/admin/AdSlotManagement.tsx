...
export default function AdSlotManagement() {
  ...
  const fetchAdSlots = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://6617dc70-8b2e-4e8f-97a1-db89d2a8d414.preview.emergentagent.com"}/api/admin/ad-slots`, {
  ...
  const handleUpdateSlot = async (slotId: string, updates: Partial<AdSlot>) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://6617dc70-8b2e-4e8f-97a1-db89d2a8d414.preview.emergentagent.com"}/api/admin/ad-slots/${slotId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
