...
export default function AdSlotManagement() {
  ...
  const fetchAdSlots = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://6f432a21-8a1b-4406-9faf-ea20a067d237.preview.emergentagent.com"}/api/admin/ad-slots`, {
  ...
  const handleUpdateSlot = async (slotId: string, updates: Partial<AdSlot>) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://6f432a21-8a1b-4406-9faf-ea20a067d237.preview.emergentagent.com"}/api/admin/ad-slots/${slotId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
