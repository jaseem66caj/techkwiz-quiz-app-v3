...
export default function AdSlotManagement() {
  ...
  const fetchAdSlots = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://306de61d-9011-4e81-a193-1382a0fe10f3.preview.emergentagent.com"}/api/admin/ad-slots`, {
  ...
  const handleUpdateSlot = async (slotId: string, updates: Partial<AdSlot>) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://306de61d-9011-4e81-a193-1382a0fe10f3.preview.emergentagent.com"}/api/admin/ad-slots/${slotId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
