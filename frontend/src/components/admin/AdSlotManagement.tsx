...
export default function AdSlotManagement() {
  ...
  const fetchAdSlots = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8001"}/api/admin/ad-slots`, {
  ...
  const handleUpdateSlot = async (slotId: string, updates: Partial<AdSlot>) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8001"}/api/admin/ad-slots/${slotId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
