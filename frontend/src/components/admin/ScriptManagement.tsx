...
export default function ScriptManagement() {
  ...
  const fetchScripts = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8001"}/api/admin/scripts`, {
  ...
  const handleUpdateScript = async (scriptId: string, updates: Partial<ScriptInjection>) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8001"}/api/admin/scripts/${scriptId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
