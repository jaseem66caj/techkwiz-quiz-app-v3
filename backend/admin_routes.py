...
async def admin_login(admin_data: AdminLogin):
    ...
    if not admin_user or not verify_password(admin_data.password, admin_user["password_hash"]):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def verify_admin_token(current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def create_category(category_data: QuizCategoryCreate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def update_category(category_id: str, category_data: QuizCategoryUpdate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def delete_category(category_id: str, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def get_questions(category: Optional[str] = None, difficulty: Optional[str] = None, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def create_question(question_data: QuizQuestionCreate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def update_question(question_id: str, question_data: QuizQuestionUpdate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def delete_question(question_id: str, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def create_script(script_data: ScriptInjectionCreate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def update_script(script_id: str, script_data: ScriptInjectionUpdate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def delete_script(script_id: str, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def create_ad_slot(slot_data: AdSlotCreate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def update_ad_slot(slot_id: str, slot_data: AdSlotUpdate, current_admin: AdminUser = Depends(get_current_admin_user)):
    """Update an ad slot."""
    database = get_db()
    ...
    result = await database.ad_slots.update_one(
        {"id": slot_id},
        {"$set": update_data}
    )
...
async def delete_ad_slot(slot_id: str, current_admin: AdminUser = Depends(get_current_admin_user)):
    """Delete an ad slot."""
    database = get_db()
...
async def import_quiz_data(data: QuizDataExport, current_admin: AdminUser = Depends(get_current_admin_user)):
    ...
    # Import categories
    if data.categories:
        await database.quiz_categories.insert_many([cat.dict() for cat in data.categories])
